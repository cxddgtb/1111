...
def main():
    ...
    try:
        ...
        final_nodes = []
        used_names = set()  # 记录已使用的节点名称，防止重名导致配置报错
        ...
        # 名称 -> 延迟 映射，供策略组排序使用
        delay_map = {n['node']['name']: n['delay'] for n in final_nodes}
        ...
        for country in countries:
            ...
            generate_clash_config(country_nodes, f"output/clash/clash_{country}.yaml", delay_map)

        # 生成全局 Top 500 配置 (按国家类型延迟最低平均加入)
        top_500_nodes = select_top_500(final_nodes)
        generate_clash_config(top_500_nodes, "output/clash/clash_global_top500.yaml", delay_map)
